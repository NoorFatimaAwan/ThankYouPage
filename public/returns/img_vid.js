...
function products_enabled() {
  const variant_titles = []
  var products_in_order = []
  var variants_in_order = []
  ...
  for(let x = 0; x < Shopify.checkout.line_items.length; x++){
    ...
    products_in_order.push(Shopify.checkout.line_items[x].title)
    variants_in_order.push(Shopify.checkout.line_items[x].variant_title)
    total_products = total_products + Shopify.checkout.line_items[x].quantity
  }
    $.ajax({
      method: "GET",
      url: `${host_url}/orders/products_choosen`,
      data: {products_in_order: products_in_order,variants_in_order: variants_in_order, shop_domain: Shopify.shop,show_response: true},
      dataType: "jsonp",
